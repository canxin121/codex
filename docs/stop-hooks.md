...
This is a **generic plugin mechanism**: any external script can participate. A Ralph‑loop is just one possible consumer.
...
[stop_hooks.sources.ralph]
command = "/abs/path/ralph-loop.sh"
timeout = 30
order = 10
...
## Ralph Loop Example (Conceptual)

See `docs/examples/stop-hooks/ralph-loop.sh` and `docs/examples/stop-hooks/hooks.json` for a runnable example.

A Ralph loop can be implemented as an external script that:
1) Reads/writes a state file (e.g., `.codex/ralph-loop.local.md`) to track iterations
2) Checks the last assistant message for `<promise>...</promise>`
3) Returns `block` with the same prompt until complete
